...
const ideas = [
  { idea: "AI onboarding concierge for HR teams that turns SOPs into step-by-step playbooks.", score: 88 },
  { idea: "Invoice automation for creative agencies with auto follow-ups and cashflow forecasts.", score: 76 },
  { idea: "Voice-first CRM notes for field sales reps that syncs to Salesforce.", score: 69 },
  { idea: "Smart meal planning with grocery bundling for busy professionals.", score: 58 },
  { idea: "AI resume and portfolio builder for early career professionals.", score: 81 },
  { idea: "Notion OKR plugin with progress intelligence and weekly check-ins.", score: 63 },
  { idea: "AI design critique assistant for product teams with iteration summaries.", score: 72 },
  { idea: "Personalized micro-learning for finance teams with compliance tracking.", score: 67 },
  { idea: "SMB pricing optimizer that recommends packaging based on churn signals.", score: 74 },
  { idea: "Community concierge that pairs experts with founders inside private groups.", score: 54 },
  { idea: "Zero-shot competitor intelligence dashboard for seed-stage startups.", score: 79 },
  { idea: "AI meeting follow-up assistant that generates next steps and owners.", score: 83 },
  { idea: "B2B renewal risk detector using product usage and NPS signals.", score: 71 },
  { idea: "Creator sponsorship marketplace with intent-based matching.", score: 47 },
  { idea: "AI workflow builder for operations teams with prebuilt templates.", score: 92 },
  { idea: "Localization QA copilot for mobile apps with regression alerts.", score: 61 },
  { idea: "Remote team ritual generator that automates culture check-ins.", score: 52 },
  { idea: "AI-powered legal doc summarizer for in-house counsels.", score: 77 },
  { idea: "Lead qualification copilot for inbound SaaS forms.", score: 66 },
  { idea: "Investor update generator that tracks milestones and metrics.", score: 59 },
];
...
async function run() {
  ...
  const existingUser = userList.users.find((user) => user.email === TARGET_EMAIL);
  ...
  const validationIds = validations?.map((row) => row.id) ?? [];
  if (validationIds.length) {
    await supabase.from("shared_reports").delete().in("validation_id", validationIds);
  }
